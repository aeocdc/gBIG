...
class gBIGForm(forms.Form):
    ...
    referenceFile8Upload=forms.FileField(label="reference genome file", required=False,)
    userEmail=forms.EmailField(label="Email address",required=True,help_text="email address",)
    modelUseType=forms.ChoiceField(label="model use type",
                                    choices=(("select","Select reference genome"),("upload","Upload reference genome"),),
                                    initial="select",
                                    required=True,
                                    help_text="upload or select reference genome",
                                    widget=forms.RadioSelect(attrs={"onclick":"modelUseTypeClick(this)"}),)#type
